...
const generateHLS = (filePath, outputDir) => {
  ...
  return new Promise((resolve, reject) => {
    ffmpeg(filePath)
      ...
      .on('error', (err) =>
        reject(new ApiError(500, `HLS generation failed: ${err.message}`))
      )
      .run()
  })
}
...
const uploadHLSFiles = async (hlsDir, videoId) => {
  ...
  const masterPlaylistUrl = results.find((res) =>
    res.public_id.endsWith('playlist')
  )?.secure_url
...
const generateWebPPreview = (filePath, outputWebpPath, duration) => {
  ...
  return new Promise((resolve, reject) => {
    ffmpeg(filePath)
      ...
      .on('error', (err) =>
        reject(
          new ApiError(500, `WebP preview generation failed: ${err.message}`)
        )
      )
      .run()
  })
...
const generateWaveform = (filePath, outputWaveformPath) => {
  return new Promise((resolve, reject) => {
    ffmpeg(filePath)
      .outputOptions([
        '-filter_complex',
        'aformat=channel_layouts=mono,compand,showwavespic=s=1200x120:colors=cyan',
        '-frames:v 1'
      ])
      .output(outputWaveformPath)
      .on('end', () => resolve('Waveform generation completed.'))
      .on('error', (err) =>
        reject(new ApiError(500, `Waveform generation failed: ${err.message}`))
      )
      .run()
  })
}
...
const generateSpriteSheetAndVtt = (filePath, outputDir, duration) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    ffmpeg(filePath)
      ...
      .on('end', async () => {
        try {
          ...
          reject(
            new ApiError(
              500,
              `VTT file generation failed: ${vttError.message}`
            )
          )
        }
      })
      .on('error', (err) =>
        reject(
          new ApiError(500, `Sprite sheet generation failed: ${err.message}`)
        )
      )
      .run()
  })
...
const detectIntro = (filePath, outputTxtPath) => {
  return new Promise((resolve, reject) => {
    ffmpeg(filePath)
      ...
      .on('error', (err) =>
        reject(new ApiError(500, `Scene detection failed: ${err.message}`))
      )
      .on('data', (chunk) => {
...
const parseIntroTimestamps = async (filePath) => {
  ...
  const potentialIntroEnd = timestamps.find(
    (t) => t > potentialIntroStart + 5 && t < 180
  )
...
const processVideo = async () => {
  ...
  try {
    ...
    const masterPlaylistUrl = await uploadHLSFiles(
      hlsOutputDir,
      video._id.toString()
    )
    ...
    webpTempPath = path.join(
      projectRoot,
      'BACKEND',
      'public',
      'temp',
      `${video._id}-preview.webp`
    )
    await generateWebPPreview(videoLocalPath, webpTempPath, duration)
    const webpUploadResponse = await uploadOnCloudinary(webpTempPath, {
      public_id: `previews/${video._id}`,
      resource_type: 'image'
    })
    if (webpUploadResponse) {
      video.previewAnimationUrl = webpUploadResponse.secure_url
    }
    console.log('WebP preview generated and uploaded.')

    // 4. Generate and upload Waveform image
    console.log('Generating waveform image...')
    waveformTempPath = path.join(
      projectRoot,
      'BACKEND',
      'public',
      'temp',
      `${video._id}-waveform.png`
    )
    await generateWaveform(videoLocalPath, waveformTempPath)
    const waveformUploadResponse = await uploadOnCloudinary(waveformTempPath, {
      public_id: `waveforms/${video._id}`,
      resource_type: 'image'
    })
    if (waveformUploadResponse) {
      video.waveformUrl = waveformUploadResponse.secure_url
    }
    ...
    const spriteSheetUploadResponse = await uploadOnCloudinary(
      path.join(spriteSheetDir, 'sprite.jpg'),
      { public_id: `sprites/${video._id}/sprite`, resource_type: 'image' }
    )
    const vttUploadResponse = await uploadOnCloudinary(
      path.join(spriteSheetDir, 'sprite.vtt'),
      { public_id: `sprites/${video._id}/sprite`, resource_type: 'raw' }
    )
    if (spriteSheetUploadResponse) {
      video.spriteSheetUrl = spriteSheetUploadResponse.secure_url
    }
    if (vttUploadResponse) {
      video.spriteSheetVttUrl = vttUploadResponse.secure_url
    }
    console.log('Sprite sheet and VTT generated and uploaded.')

    // 6. Detect Intro
    console.log('Detecting intro...')
    sceneChangesPath = path.join(
      projectRoot,
      'BACKEND',
      'public',
      'temp',
      `${video._id}-scenes.txt`
    )
    ...
    if (video) {
      await fs
        .unlink(video.videoFiles)
        .catch((e) => console.error('Error deleting video file:', e))
      if (video.thumbnail && video.thumbnail.startsWith(projectRoot)) {
        await fs
          .unlink(video.thumbnail)
          .catch((e) => console.error('Error deleting thumbnail file:', e))
      }
    }
    if (hlsOutputDir) {
      await fs
        .remove(hlsOutputDir)
        .catch((e) => console.error('Error deleting HLS directory:', e))
    }
    if (webpTempPath) {
      await fs
        .unlink(webpTempPath)
        .catch((e) => console.error('Error deleting WebP preview file:', e))
    }
    if (waveformTempPath) {
      await fs
        .unlink(waveformTempPath)
        .catch((e) => console.error('Error deleting waveform file:', e))
    }
    if (spriteSheetDir) {
      await fs
        .remove(spriteSheetDir)
        .catch((e) =>
          console.error('Error deleting sprite sheet directory:', e)
        )
    }
    if (sceneChangesPath) {
      await fs
        .unlink(sceneChangesPath)
        .catch((e) => console.error('Error deleting scene changes file:', e))
    }
