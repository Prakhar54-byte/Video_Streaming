 ...
 */
export const generateSpriteSheetAndVtt = async (
  inputPath,
  outputDir,
  options = {}
) => {
  ...
  const absoluteOutDir = path.isAbsolute(outputDir)
    ? outputDir
  ...
  const estimated =
    duration > 0 ? Math.floor(duration / intervalSeconds) + 1 : 1
  ...
  await new Promise((resolve, reject) => {
    ffmpeg(inputPath)
      .outputOptions(['-vf', vf, '-frames:v 1', '-q:v 3', '-y'])
 ...
 */
export const generateHLSPlaylist = async (
  inputPath,
  outputDir,
    ...
    try {
      ...
      if (targetSizeMB) {
        ...
        videoBitrate = Math.floor(
          (targetSizeBytes * 8) / duration / 1000 - audioBitrate
        )
      } else {
      ...
      ffmpeg(inputPath)
        ...
        .audioBitrate('128k')
        .outputOptions(['-preset medium', '-crf 28', '-movflags +faststart'])
        .on('progress', (progress) => {
