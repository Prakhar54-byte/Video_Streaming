...
const getAllVideos = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, { videos }, 'All videos get'))
  } catch (error) {
    throw new ApiError(
      400,
      error?.message.toString() || 'Some error in getAllVideos'
    )
  }
})
...
const getVideoById = asyncHandler(async (req, res) => {
  try {
    ...
    video.waveformUrl = video.waveformUrl
      ? toPublicUrlPath(video.waveformUrl)
      : ''
    video.spriteSheetUrl = video.spriteSheetUrl
      ? toPublicUrlPath(video.spriteSheetUrl)
      : ''
    video.spriteSheetVttUrl = video.spriteSheetVttUrl
      ? toPublicUrlPath(video.spriteSheetVttUrl)
      : ''

    video.hlsMasterPlaylist =
      isReady && video.hlsMasterPlaylist
        ? toPublicUrlPath(video.hlsMasterPlaylist)
        : ''

    // If HLS points to a local file that doesn't exist, don't advertise it.
    if (
      video.hlsMasterPlaylist &&
      !/^https?:\/\//i.test(video.hlsMasterPlaylist)
    ) {
...
const updateVideo = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, { videoId, updatedVideo }, 'Video updated'))
  } catch (error) {
...
const deleteVideo = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, { videoId, video }, 'Video deleted'))
  } catch (error) {
...
const togglePublishStatus = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, { videoId, video }, 'Video publish status toggled')
      )
  } catch (e) {
...
const homepageVideos = asyncHandler(async (req, res) => {
  try {
    ...
    throw new ApiError(
      500,
      error?.message || 'Failed to fetch homepage videos'
    )
  }
})
...
const addToWatchHistory = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json(new ApiResponse(500, null, 'Failed to add video to watch history'))
  }
})
