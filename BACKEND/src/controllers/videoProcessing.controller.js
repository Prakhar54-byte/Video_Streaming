...
try {
  const kafkaModule =
    await import('../../ingestion/kafka-producers/videoEventProducer.js')
...
export const processUploadedVideo = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { status: 'processing' }, 'Video processing started')
    )
})
...
async function resolveInputVideoPath ({ videoFiles, workDir, publicRoot }) {
  ...
  if (isRemoteUrl(raw)) {
    ...
    if (!response.ok || !response.body) {
      throw new ApiError(
        400,
        `Failed to download remote video: ${response.status}`
      )
    }
...
async function processVideoInBackground (videoId, videoUrl) {
  try {
    ...
    // 2. Extract metadata
    const metadata =
      await videoProcessingService.extractVideoMetadata(inputPath)
    ...
    video.thumbnail =
      mainThumbUrl || toPublicRel(publicRoot, mainThumbnailPath)
    ...
    const waveformPath = path.join(
      publicRoot,
      'temp',
      `waveform-${videoId}.png`
    )
    let waveformUrl
    try {
      await videoProcessingService.generateWaveformImage(
        inputPath,
        waveformPath,
      ...
      console.warn(
        `Waveform generation failed for video ${videoId}:`,
        e?.message || e
      )
    ...
    try {
      const result = await videoProcessingService.generateSpriteSheetAndVtt(
        inputPath,
        spritesDir,
        {
          intervalSeconds: 10,
      ...
      console.warn(
        `Sprite/VTT generation failed for video ${videoId}:`,
        e?.message || e
      )
    ...
    try {
      ...
      const pythonScriptPath = path.resolve(
        backendRoot,
        '../processing/intro-detection/detect_intro.py'
      )
      ...
      if (
        mlResult &&
        mlResult.introStartTime !== undefined &&
        mlResult.introEndTime !== undefined
      ) {
        video.introStartTime = mlResult.introStartTime
        video.introEndTime = mlResult.introEndTime
        console.log(
          `[ML] Intro detected and set: ${video.introStartTime} - ${video.introEndTime}`
        )
      } else {
      ...
      console.log(
        '[ML] Applying FALLBACK intro timings (10s - 40s) for demonstration.'
      )
...
export const compressVideoEndpoint = asyncHandler(async (req, res) => {
  ...
  const outputPath = path.join(
    tempDir,
    `compressed_${videoId}_${Date.now()}.mp4`
  )
