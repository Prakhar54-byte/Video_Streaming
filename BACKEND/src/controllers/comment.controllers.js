...
const addComment = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json(
        new ApiResponse(201, { videoId, comment }, 'Comment added successfully')
      )
  } catch (error) {
...
const updateComment = asyncHandler(async (req, res) => {
  try {
    ...
    if (!comment) {
      throw new ApiError(
        404,
        'Comment not found or you are not authorized to update'
      )
    }
...
const deleteComment = asyncHandler(async (req, res) => {
  try {
    ...
    if (!comment) {
      throw new ApiError(
        404,
        'Comment not found or you are not authorized to delete'
      )
    }
