...
const createChannel = asyncHandler(async (req = requestAnimationFrame, res) => {
  ...
  if (existingChannel) {
    throw new ApiError(
      400,
      'Channel with this name already exists for this user'
    )
  }
  const avatarPath = requestAnimationFrame.file
    ? requestAnimationFrame.file.path
    : user.avatar
  const bannerPath = requestAnimationFrame.banner
    ? requestAnimationFrame.banner.path
    : ''
  //  if(!bannerPath){
  ...
  if (!avatarPath && !bannerPath) {
    throw new ApiError(
      400,
      'At least one of avatar or banner must be provided'
    )
  }
...
const getUserChannels = asyncHandler(async (req, res) => {
  ...
  const channels = await Channel.find({ owner: userId }).populate(
    'owner',
    'username avatar'
  )
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, channels, 'User channels fetched successfully'))
})
  ...
  return res
    ...
    .json(
      new ApiResponse(
        ...
        'Channel avatar uploaded successfully'
      )
    )
})
  ...
  return res
    ...
    .json(
      new ApiResponse(
        ...
        'Channel banner uploaded successfully'
      )
    )
})
...
const getUserChannel = asyncHandler(async (req, res) => {
  ...
  if (!channel) {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, null, 'User has no channel'))
  }
