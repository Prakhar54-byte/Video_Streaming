...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
  try {
    ...
    throw new ApiError(
      400,
      error?.message || 'Some error in addVideoToPlaylist '
    )
  }
})
...
const removeVideoFromPlaylist = asyncHandler(async (req, res) => {
  try {
    ...
    throw new ApiError(
      400,
      error?.message || 'Some error in removeVideoFromPlaylist'
    )
  }
})
...
const updatePlaylist = asyncHandler(async (req, res) => {
  try {
    ...
    if (!name || !description) {
      throw new ApiError(
        400,
        'Playlist name and description is do not exist to update'
      )
    }
