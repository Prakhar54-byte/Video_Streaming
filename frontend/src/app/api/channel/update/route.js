...
export async function PATCH(request) {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ message: errorData.message || "Failed to update channel" }, { status: response.status })
    }

    const data = await response.json()
    return NextResponse.json({ message: "Channel updated successfully", channel: data.data })
  } catch (error) {
