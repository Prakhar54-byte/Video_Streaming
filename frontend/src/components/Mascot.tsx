  ...
  return (
    <div className="flex flex-col items-center justify-center p-4">
      <div className="relative group cursor-pointer">
        {/* Glow effect */}
        <div className="absolute inset-0 bg-[#32FF7E] blur-2xl opacity-20 rounded-full animate-pulse group-hover:opacity-40 transition-opacity"></div>
        ...
        {/* Level Badge */}
        <div className="absolute -bottom-2 -right-2 bg-[#32FF7E] text-black text-xs font-bold px-2 py-0.5 rounded-full border-2 border-[#080a0f] z-20">
          Lv.{level}
        </div>
      </div>
      
      <div className="mt-6 bg-white/5 border border-white/10 px-4 py-1.5 rounded-full flex items-center gap-2">
        <div className={`w-2 h-2 rounded-full ${state === 'idle' ? 'bg-blue-400' : 'bg-[#32FF7E] animate-ping'}`}></div>
