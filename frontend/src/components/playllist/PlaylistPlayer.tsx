...
import { Play, SkipForward, SkipBack, Shuffle, Repeat, List, X } from "lucide-react"
...
export function PlaylistPlayer({
  ...
  useEffect(() => {
    if (playlist && currentVideoId) {
      const index = playlist.videoDetails.findIndex((video) => video._id === currentVideoId)
      if (index !== -1) {
  ...
  const fetchPlaylist = async () => {
    try {
      ...
      if (response.success && response.data && response.data.length > 0) {
        ...
        const order = Array.from({ length: response.data[0].videoDetails.length }, (_, i) => i)
  ...
  const getVideoOrder = () => {
    if (!playlist) return []
    return isShuffled ? shuffledOrder : Array.from({ length: playlist.videoDetails.length }, (_, i) => i)
  }
  ...
  const toggleShuffle = () => {
    ...
    if (!isShuffled) {
      ...
      const order = Array.from({ length: playlist.videoDetails.length }, (_, i) => i)
    ...
    <Card className={className}>
      ...
      <CardContent className="p-4 pt-0">
        ...
        {currentVideo && (
          <div className="mb-4 p-3 bg-muted rounded-lg">
            <div className="flex items-start gap-3">
              <img
                src={currentVideo.thumbnail || "/placeholder.svg?height=48&width=80"}
        ...
        <div className="flex items-center justify-center gap-2 mb-4">
          <Button variant="ghost" size="sm" onClick={toggleShuffle} className={isShuffled ? "text-primary" : ""}>
          ...
          <Button
            ...
            size="sm"
            onClick={playNext}
            disabled={currentOrderIndex === order.length - 1 && repeatMode !== "all"}
          >
              ...
              return (
                <div
                  ...
                  className={`flex items-start gap-3 p-2 rounded cursor-pointer transition-colors ${
                    isCurrentVideo ? "bg-primary/10 border border-primary/20" : "hover:bg-muted/50"
                  }`}
                  onClick={() => {
                ...
                >
                  <div className="flex-shrink-0 text-xs text-muted-foreground w-6 text-center">{orderIndex + 1}</div>

                  <img
                    src={video.thumbnail || "/placeholder.svg?height=32&width=56"}
                    alt={video.title}
                    className="w-12 h-8 object-cover rounded flex-shrink-0"
                  />

                  <div className="flex-1 min-w-0">
                    <h5 className={`text-xs line-clamp-2 ${isCurrentVideo ? "font-medium text-primary" : ""}`}>
                      {video.title}
                    </h5>
                    {video.duration && (
                  ...
                  {isCurrentVideo && <Play className="h-3 w-3 text-primary flex-shrink-0" />}
                </div>
              )
            })}
          </div>
