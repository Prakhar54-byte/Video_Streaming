...
export default function VideoPage() {
  ...
  useEffect(() => {
    ...
    const fetchVideo = async () => {
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/videos/${id}`)
  ...
  const videoUrl = video ? `${process.env.NEXT_PUBLIC_API_URL}/videos/${id}/stream` : ''
  const {
    videoRef,
    isPlaying,
    progress,
  ...
  if (error || playerError) return <div className="text-center py-8 text-red-500">{error || playerError}</div>
