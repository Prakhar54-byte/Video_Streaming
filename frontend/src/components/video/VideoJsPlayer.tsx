...
export function VideoJsPlayer({
  ...
  useEffect(() => {
    ...
    console.log(
      "[VideoJS] Skip Intro Config:",
      JSON.stringify({ introStartTime, introEndTime }),
    );
    ...
    try {
      if (typeof videojsLib !== "function") {
        throw new Error(
          `videojs import is not a function (got: ${typeof videojsLib})`,
        );
      }
      ...
      player.ready(() => {
        console.log(
          "[VideoJS] Player ready, tech:",
          player.tech({ IWillNotUseThisInPlugins: true })?.name,
        );
      });
  ...
  return (
    <div
      ref={containerRef}
      data-vjs-player
      className="relative w-full h-full bg-black rounded-xl overflow-hidden aspect-video shadow-2xl ring-1 ring-white/10"
    >
